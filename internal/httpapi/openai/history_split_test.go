...
func TestApplyCurrentInputFileUploadsToolsContextSeparately(t *testing.T) {
	...
	if ds.uploadCalls[1].Filename != "FUNCTION_DEFINITIONS.txt" {
		t.Fatalf("expected second upload to be FUNCTION_DEFINITIONS.txt, got %q", ds.uploadCalls[1].Filename)
	}
	...
	for _, want := range []string{"# FUNCTION_DEFINITIONS.txt", "Tool: search", "Description: search docs", `Parameters: {"type":"object"}`} {
	...
	if !strings.Contains(out.FinalPrompt, "Continue from the latest state in the attached CONVERSATION_CONTEXT.txt context.") || !strings.Contains(out.FinalPrompt, "FUNCTION_DEFINITIONS.txt") {
	...
	if !strings.Contains(out.PromptTokenText, "# CONVERSATION_CONTEXT.txt") || !strings.Contains(out.PromptTokenText, "# FUNCTION_DEFINITIONS.txt") || !strings.Contains(out.PromptTokenText, "Description: search docs") {
...
func TestResponsesCurrentInputFileUploadsToolsSeparately(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "CONVERSATION_CONTEXT.txt" || ds.uploadCalls[1].Filename != "FUNCTION_DEFINITIONS.txt" {
	...
	toolsText := string(ds.uploadCalls[1].Data)
	if !strings.Contains(toolsText, "# FUNCTION_DEFINITIONS.txt") || !strings.Contains(toolsText, "Tool: search") || !strings.Contains(toolsText, "Description: search docs") {
		t.Fatalf("expected tools transcript to include schema, got %q", toolsText)
	}
	promptText, _ := ds.completionReq["prompt"].(string)
	if !strings.Contains(promptText, "FUNCTION_DEFINITIONS.txt") || !strings.Contains(promptText, "TOOL CALL FORMAT") {
