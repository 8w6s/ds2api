...
func TestHandleVercelStreamSwitchReuploadsCurrentInputFile(t *testing.T) {
	...
	stdReq := promptcompat.StandardRequest{
		...
		FinalPrompt:             "Continue from the latest state in the attached CONVERSATION_CONTEXT.txt context. Available tool descriptions and parameter schemas are attached in FUNCTION_DEFINITIONS.txt; use only those tools and follow the tool-call format rules in this prompt.",
		PromptTokenText:         "# CONVERSATION_CONTEXT.txt\n\n=== 1. USER ===\nhello\n\n# FUNCTION_DEFINITIONS.txt\nAvailable tool descriptions and parameter schemas for this request.\n\nYou have access to these tools:\n\nTool: search\nDescription: search docs\nParameters: {\"type\":\"object\"}\n",
	...
	promptText, _ := payload["prompt"].(string)
	if !strings.Contains(promptText, "FUNCTION_DEFINITIONS.txt") {
