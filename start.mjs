...
// 编译产物路径
const BINARY = join(__dirname, isWindows ? 'neutronapi.exe' : 'neutronapi');
...
const CONFIG = {
  ...
  adminKey: process.env.NEUTRON_ADMIN_KEY || 'admin',
  webuiDir: join(__dirname, 'webui'),
  staticAdminDir: process.env.NEUTRON_STATIC_ADMIN_DIR || join(__dirname, 'static', 'admin'),
};
...
async function buildBackend() {
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn('go', ['build', '-o', BINARY, './cmd/neutronapi'], {
      cwd: __dirname,
      stdio: 'inherit',
      shell: isWindows,
...
async function startBackendDev() {
  ...
  const proc = spawn('go', ['run', './cmd/neutronapi'], {
    cwd: __dirname,
    stdio: 'inherit',
    shell: isWindows,
    env: { ...process.env,
      ...
      NEUTRON_ADMIN_KEY: CONFIG.adminKey,
...
async function showMenu() {
  ...
  console.log(`  NEUTRON_ADMIN_KEY:  ${colors.cyan}${CONFIG.adminKey}${colors.reset}`);
  ...
  console.log(`${colors.dim}  自定义: NEUTRON_ADMIN_KEY=密钥 PORT=5001 node start.mjs${colors.reset}`);
...
async function main() {
  ...
  switch (cmd) {
    ...
    case '--help':
      console.log(`
${colors.bright}NeutronAPI 启动脚本 (Go)${colors.reset}
...
${colors.cyan}常用环境变量:${colors.reset}
  ...
  NEUTRON_ADMIN_KEY   管理员密钥 (默认: admin)
  NEUTRON_CONFIG_PATH 配置文件路径 (默认: config.json)
  GOPROXY            Go 模块代理 (默认: https://goproxy.cn,direct)
  NPM_REGISTRY       npm 镜像源 (默认: https://registry.npmmirror.com)

${colors.cyan}示例:${colors.reset}
  NEUTRON_ADMIN_KEY=mykey PORT=8080 node start.mjs dev
