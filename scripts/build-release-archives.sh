...
build_one() {
  ...
  if [[ "$goarm" == "-" ]]; then
    CGO_ENABLED=0 GOOS="$goos" GOARCH="$goarch" \
      go build -buildvcs=false -trimpath -ldflags="-s -w -X neutronapi/internal/version.BuildVersion=${build_version}" -o "${stage}/${bin}" ./cmd/neutronapi
  else
