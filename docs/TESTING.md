...
如果你只想跳过这些 preflight 检查，可以直接运行 `go run ./cmd/neutronapi-tests --no-preflight`。
...
| `--admin-key` | Admin 密钥 | `NEUTRON_ADMIN_KEY` 环境变量，回退 `admin` |
...
```bash
go run ./cmd/neutronapi-tests --no-preflight
```
